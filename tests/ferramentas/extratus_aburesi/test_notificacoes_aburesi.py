...
def test_inconsistencia_de_triagem_vira_notificacao(limpar_notificacoes_teste):
    ...
    # dispensa lógica por pessoa cobre os 4 tipos de "Ferramentas".
    # `descartavel`/`resolver` são montados pelo agregador, não aqui.
...
def test_sucesso_do_robo_vira_notificacao_com_chave_em_ferramentas(limpar_notificacoes_teste):
    ...
    # LÓGICA por pessoa — "pronto" aqui também carrega `chave`, igual
    # erro/revisão/triagem (descartavel/resolver são montados pelo
    # agregador, não aqui).
    job = registrar_processado(
