...
def test_aprovar_manualmente_segunda_chamada_nao_reprocessa():
    """Ver teste equivalente em tests/ferramentas/extratus/test_triagem_manual.py
    — mesma trava contra clique duplo em "Aprovar"."""
    registro = _criar("teste_triagem_aprovar_duplo_clique_aburesi.pdf")
    db_triagem.atualizar_apos_triagem(registro.id, db_triagem.NAO_ENCONTRADO, None, "revisao", "não achou nada")

    primeira = db_triagem.aprovar_manualmente(registro.id, processo_manual="9990001-01.2026.8.00.1001")
    segunda = db_triagem.aprovar_manualmente(registro.id, processo_manual="9990001-01.2026.8.00.1001")
    ...
    assert do_banco.status == db_triagem.PROCESSANDO
...
def test_atualizar_apos_triagem_processando_com_processo_ja_ativo_vira_duplicado():
    """Ver teste equivalente em tests/ferramentas/extratus/test_triagem_manual.py
    — mesma trava real de banco (índice único parcial)."""
    processo = "9990002-02.2026.8.00.1002"
    primeiro = _criar("teste_triagem_processo_ativo_1_aburesi.pdf")
    segundo = _criar("teste_triagem_processo_ativo_2_aburesi.pdf")
    ...
    assert do_banco_primeiro.status == db_triagem.PROCESSANDO
...
def test_aprovar_manualmente_com_processo_ja_ativo_em_outro_arquivo_vira_duplicado():
    processo = "9990003-03.2026.8.00.1003"
    ja_processando = _criar("teste_triagem_processo_ativo_aprovar_1_aburesi.pdf")
    aguardando_conferencia = _criar("teste_triagem_processo_ativo_aprovar_2_aburesi.pdf")
...
def test_listar_erros_do_usuario_traz_so_erro_do_proprio_usuario():
    erro_a = _criar("teste_triagem_minhas_erro_a_aburesi.pdf", USUARIO_A)
    db_triagem.marcar_erro(erro_a.id, "Falha ao gerar o relatório.")
    erro_b = _criar("teste_triagem_minhas_erro_b_aburesi.pdf", USUARIO_B)
    db_triagem.marcar_erro(erro_b.id, "Falha ao gerar o relatório.")
    pendente_a = _criar("teste_triagem_minhas_pendente_a_aburesi.pdf", USUARIO_A)
