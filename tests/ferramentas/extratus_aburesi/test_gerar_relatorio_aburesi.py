...
NOME_USUARIO_A = "teste_inbox_usuario_a_aburesi"
...
def clientes_logados():
    ...
    criar_usuario(
        ...
        email="teste_inbox_a_aburesi@example.com", senha=SENHA, eh_admin=True,
    )
    criar_usuario(
        nome="Teste Inbox B", nome_usuario=NOME_USUARIO_B,
        email="teste_inbox_b_aburesi@example.com", senha=SENHA, eh_admin=True,
    )
...
def test_pagina_inicial_exige_login():
    ...
    resp = cliente.get("/extratus-aburesi/fila-urgentes", follow_redirects=False)
...
def test_estado_exige_login():
    ...
    resp = cliente.get("/extratus-aburesi/fila-urgentes/estado", follow_redirects=False)
...
def test_upload_recusa_mais_de_5_arquivos(clientes_logados, tmp_path, limpar_triagem_teste):
    ...
    with patch.object(gerar_relatorio, "carregar_config", return_value={"pasta_entrada": str(tmp_path)}):
        ...
        resp = cliente_a.post("/extratus-aburesi/fila-urgentes/upload", files=arquivos)
...
def test_upload_cria_registros_e_agenda_processamento(clientes_logados, tmp_path, limpar_triagem_teste):
    ...
    with patch.object(
        ...
        resp = cliente_a.post("/extratus-aburesi/fila-urgentes/upload", files=arquivos, follow_redirects=False)
...
def test_estado_so_mostra_registros_do_proprio_usuario(clientes_logados, limpar_triagem_teste):
    ...
    resp = cliente_a.get("/extratus-aburesi/fila-urgentes/estado")
...
def test_conferencia_aprovar_dispara_geracao_em_segundo_plano(clientes_logados, limpar_triagem_teste):
    ...
    with patch.object(gerar_relatorio, "retomar_apos_conferencia") as retomar_mock:
        resp = cliente_a.post(
            f"/extratus-aburesi/fila-urgentes/conferencia/{registro.id}/aprovar", follow_redirects=False,
        )
...
def test_conferencia_aprovar_sem_processo_quando_nao_encontrado_falha(clientes_logados, limpar_triagem_teste):
    ...
    resp = cliente_a.post(f"/extratus-aburesi/fila-urgentes/conferencia/{registro.id}/aprovar", follow_redirects=False)
...
def test_conferencia_descartar_apaga_registro(clientes_logados, tmp_path, limpar_triagem_teste):
    ...
    resp = cliente_a.post(f"/extratus-aburesi/fila-urgentes/conferencia/{registro.id}/descartar", follow_redirects=False)
...
def test_usuario_nao_ve_conferencia_de_outro(clientes_logados, limpar_triagem_teste):
    ...
    resp = cliente_a.post(f"/extratus-aburesi/fila-urgentes/conferencia/{registro_b.id}/descartar", follow_redirects=False)
...
def test_pagina_inicial_mostra_badge_ambar_ate_resolver(clientes_logados, limpar_triagem_teste):
    # Ver comentário equivalente em tests/ferramentas/extratus/test_gerar_relatorio.py
    # — mesma lógica.
    cliente_a, _ = clientes_logados
    usuario_a_id = _usuario_id(NOME_USUARIO_A)
    ...
    primeira_visita = cliente_a.get("/extratus-aburesi/fila-urgentes")
    assert primeira_visita.status_code == 200
    assert badge_gerar_relatorio in primeira_visita.text

    segunda_visita = cliente_a.get("/extratus-aburesi/fila-urgentes")
...
NOME_COLABORADOR_PADRAO_TESTE = "teste_gerar_relatorio_colaborador_padrao_aburesi"
NOME_COLABORADOR_MANUAL_TESTE = "teste_gerar_relatorio_colaborador_manual_aburesi"
...
def cliente_colaborador_sem_acesso_manual():
    ...
    criar_usuario(
        ...
        email="teste_gerar_relatorio_colaborador_padrao_aburesi@example.com",
        senha=SENHA,
        eh_admin=False,
...
def cliente_colaborador_com_acesso_manual():
    ...
    criar_usuario(
        ...
        email="teste_gerar_relatorio_colaborador_manual_aburesi@example.com",
        senha=SENHA,
        eh_admin=False,
        ferramenta_ids=[extratus_id],
...
def test_colaborador_sem_acesso_manual_e_redirecionado_pra_fila(cliente_colaborador_sem_acesso_manual):
    ...
    resposta = cliente_colaborador_sem_acesso_manual.get("/extratus-aburesi/fila-urgentes", follow_redirects=False)
    assert resposta.status_code == 303
    assert resposta.headers["location"] == "/extratus-aburesi/fila-robo"

    resposta_seguida = cliente_colaborador_sem_acesso_manual.get("/extratus-aburesi/fila-urgentes")
...
def test_colaborador_com_acesso_manual_acessa_gerar_relatorio(cliente_colaborador_com_acesso_manual):
    resposta = cliente_colaborador_com_acesso_manual.get("/extratus-aburesi/fila-urgentes")
