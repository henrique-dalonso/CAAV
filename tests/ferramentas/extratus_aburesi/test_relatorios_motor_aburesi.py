...
NOME_USUARIO_TESTE = "teste_relatorios_motor_aburesi"
...
def cliente_logado():
    ...
    criar_usuario(
        nome="Teste Relatórios do Motor Aburesi",
        nome_usuario=NOME_USUARIO_TESTE,
        email="teste_relatorios_motor_aburesi@example.com",
        senha=SENHA,
        eh_admin=True,
    )
...
def test_pagina_relatorios_motor_exige_login():
    ...
    resp = cliente.get("/extratus-aburesi/relatorios-motor", follow_redirects=False)
...
def test_pagina_relatorios_motor_so_lista_jobs_do_motor(cliente_logado, limpar_jobs_criados):
    job_motor = registrar_processado(
        arquivo_pdf="teste_pagina_relatorios_motor_aburesi.pdf",
    ...
    job_manual = registrar_processado(
        arquivo_pdf="teste_pagina_relatorios_motor_manual_aburesi.pdf",
    ...
    resp = cliente_logado.get("/extratus-aburesi/relatorios-motor")

    assert resp.status_code == 200
    assert "teste_pagina_relatorios_motor_aburesi.pdf" in resp.text
    assert "teste_pagina_relatorios_motor_manual_aburesi.pdf" not in resp.text
...
def test_pagina_relatorios_motor_acessivel_sem_acesso_a_fila():
    ...
    resp = cliente.get("/extratus-aburesi/relatorios-motor")
