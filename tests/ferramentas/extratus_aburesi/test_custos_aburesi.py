...
NOME_ADMIN_PLATAFORMA = "teste_custos_admin_plataforma_aburesi"
NOME_ADMIN_SO_FERRAMENTA = "teste_custos_admin_ferramenta_aburesi"
SENHA = "senhaTeste123"
...
def test_pagina_custos_exige_login():
    cliente = TestClient(app)

    resp = cliente.get("/extratus-aburesi/custos", follow_redirects=False)

    assert resp.status_code == 303
    assert resp.headers["location"] == "/login"


def test_pagina_custos_admin_de_ferramenta_sem_ser_admin_plataforma_e_recusado(limpar_usuarios_teste):
    """Ver comentário equivalente em tests/ferramentas/extratus/
    test_custos.py (Extratus - Relatórios) — mesma lógica."""
    with obter_sessao() as sessao:
        aburesi_id = sessao.exec(select(Ferramenta.id).where(Ferramenta.slug == "extratus-aburesi")).first()

    criar_usuario(
        nome="Teste Custos Admin Ferramenta Aburesi",
        nome_usuario=NOME_ADMIN_SO_FERRAMENTA,
        email="teste_custos_admin_ferramenta_aburesi@example.com",
        senha=SENHA,
        eh_admin=False,
    ...
    resp = cliente.get("/extratus-aburesi/custos")

    assert resp.status_code == 403


def test_pagina_custos_admin_da_plataforma_acessa(limpar_usuarios_teste):
    criar_usuario(
        nome="Teste Custos Admin Plataforma Aburesi",
        nome_usuario=NOME_ADMIN_PLATAFORMA,
        email="teste_custos_admin_plataforma_aburesi@example.com",
        senha=SENHA,
        eh_admin=True,
    )
