...
def preservar_config_json():
    """atualizar_config_motor grava no config.json real (não há um de
...
def test_atualizar_config_motor_grava_pasta_e_provider(preservar_config_json):
    atualizar_config_motor(pasta_entrada="uma/pasta/de/teste", ia_provider="claude")

    salvo = json.loads(CONFIG_PATH.read_text(encoding="utf-8"))
    assert salvo["motor_pasta_entrada"] == "uma/pasta/de/teste"
    assert salvo["ia_provider"] == "claude"


def test_atualizar_config_motor_rejeita_provider_invalido(preservar_config_json):
    with pytest.raises(ValueError):
        atualizar_config_motor(pasta_entrada="qualquer", ia_provider="modelo-inventado")
