...
NOME_USUARIO_TESTE = "teste_fila_upload"
SENHA = "senhaTeste123"
PREFIXO_TESTE = "teste_fila_conferencia_"
...
def cliente_logado():
    ...
    criar_usuario(
        nome="Teste Fila Upload",
        nome_usuario=NOME_USUARIO_TESTE,
        email="teste_fila_upload@example.com",
        senha=SENHA,
        eh_admin=True,
    )
...
def test_upload_atrela_solicitante_direto_na_hora(cliente_logado, tmp_path):
    """Henrique, diretoria, 2026-08-27: 'na hora que a pessoa anexar os
    pdfs na fila e enviar ele já registrar, atrela àquele caso, o nome da
    pessoa' — substitui a tentativa anterior de deduzir isso depois
    (mapear_solicitantes_por_arquivo, removida). Testa o caminho real
    (rota de upload de verdade), não só a função isolada."""
    nome_arquivo = "teste_fila_upload_solicitante.pdf"
...
def test_descartar_todas_conferencias_remove_tudo(cliente_logado, limpar_conferencia_teste, tmp_path):
    # Patcheia listar_inconsistencias (não só carregar_config) de propósito
    # — sem isso, a rota descartaria QUALQUER inconsistência real que
    # estivesse pendente no banco compartilhado no momento em que a suíte
    # roda, não só as duas criadas aqui pro teste (achado testando ao vivo
    # com Playwright: um PDF de teste alheio virou uma 3ª inconsistência
    # de verdade por causa do checagem_watcher real, e "descartar todas"
    # descartou ele também).
...
def test_estado_fila_ordena_pendentes_vermelho_laranja_amarelo(cliente_logado, limpar_conferencia_teste, tmp_path):
    ...
    # prioridade esperada (zzz > aaa > mmm) — prova que é o status que
    # decide a ordem, não a ordem alfabética/do disco.
...
def test_pagina_fila_mostra_badge_ambar_ate_resolver(cliente_logado, limpar_conferencia_teste):
    ...
    # Fila do Robô (compartilhada, não filtra por usuário). Fica ligado
...
NOME_COLABORADOR_PADRAO_TESTE = "teste_fila_colaborador_padrao"
...
def cliente_colaborador_padrao():
    ...
    criar_usuario(
        ...
        email="teste_fila_colaborador_padrao@example.com",
        senha=SENHA,
        eh_admin=False,
