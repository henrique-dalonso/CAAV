...
def test_pagina_relatorios_motor_acessivel_sem_acesso_a_fila():
    ...
    a ferramenta liberada, mas sem `fila_motor`/`admin_ferramenta`."""
    with obter_sessao() as sessao:
    ...
    criar_usuario(
        nome="Teste RelMotor Sem Fila",
        nome_usuario=NOME_USUARIO_SEM_FILA,
        email="teste_relmotor_sem_fila@example.com",
        senha=SENHA,
        eh_admin=False,
...
def test_pagina_relatorios_manual_nao_lista_jobs_do_motor(cliente_logado, limpar_jobs_criados):
    job_motor = registrar_processado(
        arquivo_pdf="teste_pagina_relatorios_manual_motor.pdf",
    ...
    assert resp.status_code == 200
    assert "teste_pagina_relatorios_manual_motor.pdf" not in resp.text
