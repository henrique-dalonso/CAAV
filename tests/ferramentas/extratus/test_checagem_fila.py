...
USUARIO_A_TESTE = -9101
USUARIO_B_TESTE = -9102
...
def test_registrar_pendente_cria_linha_ja_com_solicitante(limpar_checagem_teste):
    """Achado real, Henrique 2026-08-27: direto na hora do upload, não
    precisa esperar o watcher nem deduzir depois quem enviou."""
    nome = f"{PREFIXO_TESTE}registrar_pendente_novo.pdf"

    registro = registrar_pendente(nome, USUARIO_A_TESTE)

    assert registro.nome_arquivo == nome
    assert registro.status == PENDENTE
    assert registro.solicitante_id == USUARIO_A_TESTE


def test_registrar_pendente_linha_ja_existente_preenche_solicitante_vazio(limpar_checagem_teste):
    """Caso raro: o watcher já criou a linha (sem solicitante, arquivo
    apareceu por fora do upload) antes do registrar_pendente chegar."""
    nome = f"{PREFIXO_TESTE}registrar_pendente_ja_existe.pdf"
...
def test_registrar_pendente_nunca_sobrescreve_solicitante_ja_preenchido(limpar_checagem_teste):
    nome = f"{PREFIXO_TESTE}registrar_pendente_nao_sobrescreve.pdf"

    registrar_pendente(nome, USUARIO_A_TESTE)
    registro = registrar_pendente(nome, USUARIO_B_TESTE)
...
def test_mapear_solicitantes_por_arquivo_nome_reutilizado_pega_o_envio_certo(limpar_uploads_teste):
    """Achado real, Henrique 2026-08-27: nome de arquivo genérico
    (ex: "pdf.pdf") pode se repetir ao longo do tempo — cada job precisa
    casar com o envio que realmente o originou, não com qualquer envio
    daquele nome."""
    agora = datetime.now()
...
def test_resolver_solicitantes_usa_direto_quando_ja_preenchido():
    """Relatório novo (a partir de 2026-08-27) já vem com
    solicitante_id direto — não precisa nem tentar deduzir."""
    item = SimpleNamespace(id=1, arquivo_pdf=f"{PREFIXO_TESTE}nao_deveria_importar.pdf", criado_em=datetime.now(), solicitante_id=USUARIO_A_TESTE)

    resultado = resolver_solicitantes([item])

    assert resultado == {1: USUARIO_A_TESTE}


def test_resolver_solicitantes_cai_pro_fallback_quando_direto_vazio(limpar_uploads_teste):
    """Henrique, 2026-08-27: relatório de antes da coluna
    solicitante_id existir ficou "não identificado" sem o fallback —
    aqui ele tem que voltar a aparecer, deduzido pelo upload."""
    agora = datetime.now()
    nome = f"{PREFIXO_TESTE}resolver_fallback.pdf"

    upload = _registrar_upload_com_horario(nome, USUARIO_B_TESTE, agora - timedelta(minutes=10))
    limpar_uploads_teste.append(upload.id)

    item = SimpleNamespace(id=2, arquivo_pdf=nome, criado_em=agora, solicitante_id=None)
...
def test_resolver_solicitantes_sem_nenhuma_fonte_fica_none():
    item = SimpleNamespace(id=3, arquivo_pdf=f"{PREFIXO_TESTE}resolver_sem_fonte.pdf", criado_em=datetime.now(), solicitante_id=None)

    resultado = resolver_solicitantes([item])

    assert resultado == {3: None}
...
def test_contar_inconsistencias_novas_nao_conta_o_que_ja_foi_visto(limpar_checagem_teste):
    nome = f"{PREFIXO_TESTE}badge_ja_visto.pdf"
    registro = next(p for p in _sincronizar_so_de_teste({nome}) if p.nome_arquivo == nome)
    atualizar_apos_checagem(registro.id, NAO_ENCONTRADO, None, "revisao", "não achou nada")

    depois_de_ver = datetime.now()

    assert contar_inconsistencias_novas(depois_de_ver) == 0
