...
def test_montar_diagnostico_documento_digitalizado_nao_aplica_triagem():
    # 9 de 10 páginas sem texto real (nenhuma delas passada aqui tem
    # texto de verdade) — bem acima do limite de "parece digitalizado",
    # a triagem de terceiros/quebrada nunca chega a rodar.
...
def test_montar_diagnostico_sem_paginas_suspeitas_devolve_diagnostico_original():
    ...
    diagnostico, paginas_relevantes, paginas_excluidas, _, _ = montar_diagnostico_com_triagem(
        "qualquer.pdf", paginas=paginas_fake, total_paginas=1
    )
...
def test_montar_diagnostico_resgata_pagina_problematica_com_cliente(monkeypatch):
    """Regressão do achado real (Henrique, 2026-08-26): página sem texto
    confiável, com `cliente` fornecido, é resgatada via transcrição antes
    da checagem de "parece digitalizado" — o resgate pode tirar o
    documento desse caminho por completo."""
    paginas_fake = [
        _pagina_fake_completa(1, "Vistos. Defiro o pedido de busca e apreensão do bem indicado nos autos."),
        _pagina_fake_completa(2, ""),  # sem texto nenhum — seria "digitalizado" sozinha
    ]
    ...
    assert paginas_relevantes is not None  # não caiu mais no caminho "digitalizado"


def test_montar_diagnostico_sem_cliente_nao_tenta_resgatar():
    """cliente=None (uso direto/teste, ou quando não há chave de API) —
    mantém o comportamento antigo: página problemática conta normal, sem
    tentar resgatar nada."""
