...
def test_resumo_mes_atual_soma_custo_e_quantidade_do_mes_corrente(limpar_jobs_criados):
    ...
    job1 = registrar_processado(
        arquivo_pdf="teste_mes_atual_1.pdf", processo="0000000-00.2026.8.00.0010",
        relatorio_path=None, destino_pdf=None, confianca="alta",
        uso_ia={"modelo": "claude-sonnet-5", "custo_estimado_usd": 0.30},
        usuario_id=USUARIO_TESTE_A,
    )
    job2 = registrar_processado(
        arquivo_pdf="teste_mes_atual_2.pdf", processo="0000000-00.2026.8.00.0011",
        relatorio_path=None, destino_pdf=None, confianca="alta",
...
def test_resumo_mes_atual_nao_mistura_mes_anterior_com_o_atual(limpar_jobs_criados):
    ...
    job = registrar_processado(
        arquivo_pdf="teste_mes_anterior.pdf", processo="0000000-00.2026.8.00.0012",
        relatorio_path=None, destino_pdf=None, confianca="alta",
    ...
    # Empurra o job criado pra dentro do mês anterior, direto no banco —
    # registrar_processado sempre usa a hora real (default_factory), não
    # dá pra "nascer no passado" por fora.
    agora = datetime.now()
...
def test_serie_temporal_custo_ultimo_ponto_inclui_custo_de_hoje(periodo, limpar_jobs_criados):
    ...
    job = registrar_processado(
        arquivo_pdf=f"teste_serie_{periodo}.pdf", processo="0000000-00.2026.8.00.0013",
        relatorio_path=None, destino_pdf=None, confianca="alta",
...
def test_detalhar_custo_e_quantidade_por_usuario_traz_quantidade_e_media(limpar_jobs_criados):
    ...
    job1 = registrar_processado(
        arquivo_pdf="teste_detalhe_usuario_1.pdf", processo="0000000-00.2026.8.00.0014",
        relatorio_path=None, destino_pdf=None, confianca="alta",
        uso_ia={"modelo": "claude-sonnet-5", "custo_estimado_usd": 0.40},
        usuario_id=USUARIO_TESTE_A,
    )
    job2 = registrar_processado(
        arquivo_pdf="teste_detalhe_usuario_2.pdf", processo="0000000-00.2026.8.00.0015",
        relatorio_path=None, destino_pdf=None, confianca="alta",
...
def test_resumo_por_status_com_custo_agrega_por_status(limpar_jobs_criados):
    ...
    job_sucesso = registrar_processado(
        arquivo_pdf="teste_status_sucesso.pdf", processo="0000000-00.2026.8.00.0016",
        relatorio_path=None, destino_pdf=None, confianca="alta",
        uso_ia={"modelo": "claude-sonnet-5", "custo_estimado_usd": 0.50},
        usuario_id=USUARIO_TESTE_A,
    )
    job_erro = registrar_erro(
        arquivo_pdf="teste_status_erro.pdf", processo=None, tipo_erro="erro_pdf",
...
def test_resumo_por_modelo_agrega_por_modelo(limpar_jobs_criados):
    ...
    job = registrar_processado(
        arquivo_pdf="teste_modelo_haiku.pdf", processo="0000000-00.2026.8.00.0017",
        relatorio_path=None, destino_pdf=None, confianca="alta",
...
def test_contar_relatorios_robo_novos_conta_so_usuario_id_none_do_proprio_solicitante(limpar_jobs_criados):
    # Henrique, 2026-09-02: badge da aba "Relatórios do Robô" deixou de
    # contar a fila inteira, compartilhada — só conta o que o PRÓPRIO
    # usuário pediu (Job.solicitante_id), mesma regra da aba "Minhas" do
    # sino.
    desde = datetime.now() - timedelta(seconds=1)
    ...
    job_robo_de_outro = registrar_processado(
        arquivo_pdf="teste_badge_robo_nao_conta_outro_solicitante.pdf",
    ...
    limpar_jobs_criados.extend([job_robo_sucesso.id, job_robo_revisao.id, job_robo_de_outro.id, job_manual.id])
