...
from app.ferramentas.extratus_aburesi.core.pipeline import processar_pdf
from app.ferramentas.extratus_aburesi.db.pendentes import (
...
def _redirecionar_com_erro(mensagem):
    return RedirectResponse(url=f"/extratus-aburesi/?erro={quote(mensagem)}", status_code=303)
...
async def enviar_pdf(
    ...
    return RedirectResponse(url="/extratus-aburesi/", status_code=303)


@router.post("/processar-tudo")
def processar_tudo(usuario: Usuario = Depends(exigir_acesso_ferramenta("extratus-aburesi"))):
    config = carregar_config()
