...
def alternar_motor_route():
    ...
    return RedirectResponse(
        url=f"/extratus-aburesi/motor?sucesso={quote(mensagem)}", status_code=303
    )
...
def atualizar_config_motor_route(
    ...
    try:
        ...
        return RedirectResponse(
            url=f"/extratus-aburesi/motor?erro={quote(str(erro))}", status_code=303
        )

    return RedirectResponse(
        url="/extratus-aburesi/motor?sucesso=" + quote("Configurações do motor salvas."),
        status_code=303,
    )
...
async def atualizar_prompt_motor_route(
    ...
    if not nome_seguro.lower().endswith(extensao_esperada):
        return RedirectResponse(
            url="/extratus-aburesi/motor?erro=" + quote(
    ...
    return RedirectResponse(
        url="/extratus-aburesi/motor?sucesso=" + quote("Prompt de instruções atualizado."),
        status_code=303,
    )
