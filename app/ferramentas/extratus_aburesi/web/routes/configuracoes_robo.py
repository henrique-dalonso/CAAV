...
def alternar_robo_route():
    ...
    return RedirectResponse(
        url=f"/extratus-aburesi/configuracoes-robo?sucesso={quote(mensagem)}", status_code=303
    )
...
def atualizar_config_robo_route(
    ...
    try:
        ...
        return RedirectResponse(
            url=f"/extratus-aburesi/configuracoes-robo?erro={quote(str(erro))}", status_code=303
        )

    return RedirectResponse(
        url="/extratus-aburesi/configuracoes-robo?sucesso=" + quote("Configurações do Robô salvas."),
        status_code=303,
    )
...
async def atualizar_prompt_robo_route(
    ...
    if not nome_seguro.lower().endswith(extensao_esperada):
        return RedirectResponse(
            url="/extratus-aburesi/configuracoes-robo?erro=" + quote(
    ...
    return RedirectResponse(
        url="/extratus-aburesi/configuracoes-robo?sucesso=" + quote("Prompt de instruções atualizado."),
        status_code=303,
    )
