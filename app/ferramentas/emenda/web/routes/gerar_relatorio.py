...
router = APIRouter(dependencies=[Depends(exigir_acesso_ferramenta("emenda"))])
...
def _redirecionar(erro=None, sucesso=None):
    ...
    return RedirectResponse(url=f"/emenda/fila-urgentes{query}", status_code=303)
...
def _link_relatorio_existente(registro):
    ...
    if registro.origem_duplicado == "robô":
        return "/emenda/relatorios-robo"
    return "/emenda/relatorios-urgentes"
...
def pagina_inicial(
    request: Request,
    usuario: Usuario = Depends(exigir_acesso_ferramenta("emenda")),
    erro: str | None = None,
    sucesso: str | None = None,
):
    # Ver comentário equivalente em app/ferramentas/extratus/web/routes/
    # gerar_relatorio.py (Extratus - Relatórios) — mesma lógica.
    if not usuario_tem_acesso_manual(usuario, "emenda"):
        return RedirectResponse("/emenda/fila-robo", status_code=303)

    pendentes, processando = _estado_atual(usuario.id)

    # Renderiza PRIMEIRO, marca como visto DEPOIS — ver comentário
    # equivalente em app/ferramentas/extratus/web/routes/gerar_relatorio.py.
...
def estado_processamento(usuario: Usuario = Depends(exigir_acesso_manual("emenda"))):
...
async def enviar_pdfs(
    ...
    usuario: Usuario = Depends(exigir_acesso_manual("emenda")),
):
