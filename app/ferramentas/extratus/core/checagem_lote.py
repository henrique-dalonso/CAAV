...
def analisar_pdf_isolado(caminho):
    """Roda `analisar_pdf` num processo separado — ver o motivo completo
    (bug real do GIL com `pypdf`, Henrique 2026-08-06/07) em
    `pdf_isolado.executar_isolado`. Função isolada (em vez de chamar
    `analisar_pdf` direto em `_checar_um_arquivo`) só pra manter o
    dispatch mockável nos testes, sem precisar de um processo de verdade
    rodando durante a suíte."""
