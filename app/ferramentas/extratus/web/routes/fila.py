...
def remover_varios_da_fila(
    ...
    for nome in nomes:
        ...
        # Limpa a linha de checagem NA HORA, em vez de esperar o próximo
        # ciclo do watcher (~5s) apagar sozinho (Henrique, 2026-08-08:
        # "quero que seja instantâneo"). Se o arquivo tinha uma
        # conferência aberta, remover por aqui é, na prática, um
        # descarte — só que feito por um caminho diferente do painel de
        # Conferências — então registra a decisão do mesmo jeito que o
        # Descartar dedicado registraria, pra não perder o rastro de
        # quem removeu o quê.
...
def descartar_todas_conferencias(
    ...
    (Henrique, 2026-08-07: "sei que é arriscado... precisa ter um botão
    que remova em lote caso seja tudo enviado por engano"). Mesma trava
    de confirmação da tela (data-confirm) protege contra clique acidental;
    cada arquivo ainda vira sua própria linha em RegistroConferencia,
...
def ver_pdf_conferencia(
    ...
    que faz o navegador mostrar o PDF na hora (inline) em vez de baixar
    (Content-Disposition: attachment só aparece quando `filename` é
    passado)."""
    registro = obter_registro(registro_id)
