...
def _redirecionar(erro=None, sucesso=None):
    ...
    return RedirectResponse(url=f"/condenacao/fila-urgentes{query}", status_code=303)
...
def _link_relatorio_existente(registro):
    ...
    if registro.origem_duplicado == "robô":
        return "/condenacao/relatorios-robo"
    return "/condenacao/relatorios-urgentes"
...
def pagina_inicial(
    ...
    # Ver comentário equivalente em app/ferramentas/extratus/web/routes/
    # gerar_relatorio.py (Extratus - Relatórios) — mesma lógica.
    if not usuario_tem_acesso_manual(usuario, "condenacao"):
        return RedirectResponse("/condenacao/fila-robo", status_code=303)
...
def estado_processamento(usuario: Usuario = Depends(exigir_acesso_manual("condenacao"))):
...
async def enviar_pdfs(
    ...
    usuario: Usuario = Depends(exigir_acesso_manual("condenacao")),
):
