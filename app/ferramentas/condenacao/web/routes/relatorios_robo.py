...
def _redirecionar(erro=None, sucesso=None):
    ...
    return RedirectResponse(url=f"/condenacao/relatorios-robo{query}", status_code=303)
...
def marcar_revisado_route(job_id: int, usuario: Usuario = Depends(exigir_acesso_ferramenta("condenacao"))):
    """Ver docstring equivalente em app/ferramentas/extratus/web/routes/
    relatorios_robo.py (Extratus - Relatórios) — mesma lógica."""
    if not marcar_como_revisado(job_id, usuario.id, ferramenta_slug=FERRAMENTA_SLUG_NUCLEO):
        return _redirecionar(erro="Essa condenação não existe mais, ou já não está em revisão.")
